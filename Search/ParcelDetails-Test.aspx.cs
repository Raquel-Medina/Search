...
namespace Search
{
    public partial class ParcelDetails_Test : System.Web.UI.Page
    {
        // *** Connection Strings *** //
        ...
        private void GetParcelInfo()
        {
            ...
            if (parcelID.Length == 9)
            {
                ...
                GetOwners(parcelID, book, map, parcel, split);
        ...
        private void GetOwners(string parcelID, string book, string map, string parcel, string split)
        {
            // *** Stored Procedure *** //
            string ownersProcedure = "[dbo].[Sp_Search_OwnersByParcel]";
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                con.Open();
                SqlCommand cmdOwners = new SqlCommand(ownersProcedure, con);
                SqlDataReader drOwners;

                // parcel documents
                cmdOwners.CommandType = CommandType.StoredProcedure;
                cmdOwners.Parameters.Add("@BookNumber", SqlDbType.VarChar).Value = book;
                cmdOwners.Parameters.Add("@MapNumber", SqlDbType.VarChar).Value = map;
                cmdOwners.Parameters.Add("@ParcelNumber", SqlDbType.VarChar).Value = parcel;
                cmdOwners.Parameters.Add("@SplitNumber", SqlDbType.VarChar).Value = split;

                // *** Owners *** //
                drOwners = cmdOwners.ExecuteReader();
                var dtOwner1 = new DataTable();
                var dtOwner2 = new DataTable();
                dtOwner1.Columns.Add("", typeof(System.String));
                dtOwner2.Columns.Add("", typeof(System.String));
                ...
                if (drOwners.HasRows)
                {
                    while (drOwners.Read())
                    {
                        owner1 = drOwners["NAME1"].ToString();
                        owner2 = drOwners["NAME2"].ToString();

                        dtOwner1.Rows.Add(owner1);
                        dtOwner2.Rows.Add(owner2);
                    }

                    //if (dtOwner1.Rows.Count > 1)
                    //{
                    //    lblMultipleOwner1.Text = "*Additional owners exist";
                    //}

                    gvOwner1.DataSource = dtOwner1;
                    gvOwner1.ShowHeader = false;
                    gvOwner1.DataBind();

                    gvOwner2.DataSource = dtOwner2;
                    gvOwner2.ShowHeader = false;
                    gvOwner2.DataBind();
                }
